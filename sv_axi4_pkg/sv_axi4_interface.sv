...
interface sv_axi4_interface(input logic aclk);
 ...
 // Write Address Channel (AW)-
  ...
  property p_awvalid_stable;
    @(posedge aclk) disable iff (!areset_n || !has_checks)
  ...
  assert property (@(posedge aclk) disable iff (!areset_n || !has_checks) !$isunknown(awvalid))
    else $error("[AXI4_IF] AWVALID is X/Z");
  assert property (@(posedge aclk) disable iff (!areset_n || !has_checks) !$isunknown(wvalid))
    else $error("[AXI4_IF] WVALID is X/Z");
  assert property (@(posedge aclk) disable iff (!areset_n || !has_checks) !$isunknown(arvalid))
